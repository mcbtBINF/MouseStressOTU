#Modeling Behavior (just as the taxonomic abundances) versus the experimental conditions.
...
## This is only over the "metadata", so these values don't actually matter
## FALSE is needed for otuTable to make some histograms plot correctly
otuTable <- FALSE
...
## divider <- 8
## This is the same across all the levels because the behavior data won't change
divider <- 4
if (otuTable == TRUE) {
  taxonomicLevels <- 1
} else {
...
for( t in tissues ) {
  ...
  for( taxa in taxonomicLevels) {
    ...
    write.table(dFrame, file=paste(dataType,"_analysis_BehaviorStressSexCage_gls_L", taxa, "_tissue_",t, "_divider_", divider, ".txt",sep=""), sep="\t",row.names=FALSE)
    
    ## This might be overcounting things.
    bestAt <- unique(bestAt)
    write.table(bestAt, file=paste(dataType,"_SigHitsat_BehaviorStressSexCage_gls_L", taxa, "_tissue_", t, "_divider_", divider, ".txt",sep=""), sep="\t",row.names=FALSE)
    
    
    pdf( paste(dataType, "_BehaviorStressSexCage_gls_L", taxa, "_tissue_", t, "_divider_", divider, ".pdf", sep=""))
    ...
    for (nameIter in toIter) {
      ...
      boxplot(outline=FALSE, bug ~ group, data = myFrame,  xaxt="n", las=2,
      ...
      boxplot(outline=FALSE, bug ~ sex,  xaxt="n", las=2,
      ...
      boxplot(outline=FALSE, bug ~ group, xaxt="n", las=2,
      ...
      ##         ##             boxplot(outline=FALSE, bug ~ behave*sex,
    ...
    for(histIter in iterOver) {
      hist(dFrame[,histIter], breaks=20, main=colnames(dFrame)[histIter], xlab="Unadjusted p-Value")
    }
    
    dev.off()
    
    ## Just print the best
    pdf( paste(dataType, "_SigHits_BehaviorStressSexCage_gls_L", taxa, "_tissue_", t, "_divider_", divider, ".pdf", sep=""))
    if(otuTable == TRUE) {
    ...
    for (nameIter in toIter) {
      ...
      boxplot(outline=FALSE, bug ~ group, data = myFrame, xaxt="n", las=2,
               main = paste("Wilcoxon Stress\np-value", format(bestAt[nameIter,]$"adjusted_wilcoxonStress",digits=3) ) )
      axis(1, at=c(1, 2), labels = c("Control", "Stress"), cex.axis=0.9)
      stripchart(bug ~ group,
                 data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE)
      
      boxplot(outline=FALSE, bug ~ sex, xaxt="n", las=2,
               main = paste("Wilcoxon sex\np-value", format(bestAt[nameIter,]$"adjusted_wilcoxonSex",digits=3) ) )
      axis(1, at=c(1, 2), labels = c("Female", "Male"), cex.axis=0.9)
      stripchart(bug ~ sex,
                 data = myFrame, vertical = TRUE, pch = 21, method="jitter", add=TRUE, ylab = names[index])
      ...
      boxplot(outline=FALSE, bug ~ group, las=2, xaxt='n',
               main = paste("Stress\np-value", format(bestAt[nameIter,]$"adjusted_group",digits=3) ) )
      
      axis(1, at=c(1, 2), labels = c("Control", "Stress"), cex.axis=0.9)
      stripchart(bug ~ group,
                 data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE)
      
      boxplot(outline=FALSE, bug ~ sex, las=2, xaxt='n',
               main = paste("Sex\np-value", format(bestAt[nameIter,]$"adjusted_sex",digits=3) ) )
      axis(1, at=c(1, 2), labels = c("Female", "Male"), cex.axis=0.9)
      stripchart(bug ~ sex,
                 data = myFrame, vertical = TRUE, pch = 21, method="jitter", add=TRUE, ylab = names[index])
    ...
    boxplot(outline=FALSE, bug ~ group*sex, ylab="Shannon diversity",
             main=paste("Stress*Sex interaction\np-value", format(pValsS[[indexS]]["MLMGroup:Sex"],digits=3)),
             xaxt='n')
    axis(1, at=c(1, 2, 3, 4), labels = c("Control\nFemale", "Stress\nFemale", "Control\nMale", "Stress\nMale"), cex.axis=0.9)
    stripchart(bug ~ group*sex,
               data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE )
    ...
    boxplot(outline=FALSE, bug ~ cage ,las=2, ylab="Shannon diversity",
             main=paste("cage effect p-value", format(pValsS[[indexS]]["Cage"],digits=3)), xaxt='n')
    axis(1, at=c(1, 3, 5, 7),
         labels=c("CF1", "SF1", "CM1", "SM1"), cex.axis=0.8)
    axis(1, at=c(2, 4, 6, 8),
         labels=c("CF2", "SF2", "CM2", "SM2"), cex.axis=0.8)
    ...
    dev.off()
    write.table(dMFrame, file=paste(dataType,"_MDSAxes_BehaviorStressSexCage_gls_L", taxa, "_tissue_", t, ".txt",sep=""), sep="\t",row.names=FALSE)
    ...
    if (taxa == 2 & t == "feces") {
      ...
      for (mdsI in 1:5){
        ...
        myFrame <- data.frame(bug, sex, group, cage)
        boxplot(outline=FALSE, bug ~ group, data = myFrame,
                 main = paste("Wilcoxon group p-value", format(dMFrame[mdsI,]$adjusted_wilcoxonStress,digits=3) ), ylab=paste0("MDS Axis ", mdsI) )
        stripchart(bug ~ group,
                   data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE)
        
        boxplot(outline=FALSE, bug ~ sex,
                 main = paste("Wilcoxon sexvalue", format(dMFrame[mdsI,]$adjusted_wilcoxonSex,digits=3) ), ylab=paste0("MDS Axis ", mdsI))
        stripchart(bug ~ sex,
                   data = myFrame, vertical = TRUE, pch = 21, method="jitter", add=TRUE, ylab = names[index])
        ...
        boxplot(outline=FALSE, bug ~ group,
                 main = paste("MLM Group p-value", format(dMFrame[mdsI,]$adjusted_group,digits=3) ), ylab=paste0("MDS Axis ", mdsI) )
        stripchart(bug ~ group,
                   data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE)
        ...
        boxplot(outline=FALSE, bug ~ group*sex, ylab=paste0("MDS Axis ", mdsI),
                 main=paste("group*sex interaction p-value", format(dMFrame[mdsI,]$"adjusted_group:sex",digits=3)),
                 xaxt='n')
        axis(1, at=c(1, 2, 3, 4), labels = c("Control\nFemale", "Exp.\nFemale", "Control\nMale", "Exp.\nMale"), cex.axis=0.9)
        stripchart(bug ~ group*sex,
                   data = myFrame,vertical = TRUE, pch = 21, method="jitter", add=TRUE )
  ...
  write.table(dSFrame, file=paste(dataType,"_ShannonDiversity_BehaviorStressSexCage_gls_",t, ".txt",sep=""), sep="\t",row.names=FALSE)
}